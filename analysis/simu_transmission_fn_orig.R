...
## Sample, with replacement, from distribution plus new mutations
rMydist_plus <- function(n,x,n_mut) {
  # n = number of samples
  # x = list of probabilities
  # n_mut = additional mutations 
...
## General model: parameters taken from data_fit.R 
gen_mod_day <- function(day,l_x){
  x <- seq(0,l_x,1)
...
simu_runs_orig <- function(timesv, mu, npat, nruns, same, randt){
  ### timesv = times vector
  ### mu = mutation rate
